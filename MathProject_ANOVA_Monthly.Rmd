...
treasury10Y_m = subset(treasuryRates, select=c(Date,Value))
colnames(treasury10Y_m)[c(1,2)] = c("Date", "Value") # rename columns
treasury10Y_m = treasury10Y[complete.cases(treasury10Y_m), ] # drop NAs
treasury10Y_m = treasury10Y[(order(as.Date(treasury10Y_m$Date))),]
row.names(treasury10Y_m) = NULL  # re-index the new subset
rates_m = treasury10Y_m[((nrow(treasury10Y_m))-((252*30)+1)):(nrow(treasury10Y_m)),] # take ~30 past years of treasury data, data takes a year to = 252 days per year and 30 years of past data from the latest observed value
rownames(rates_m) = NULL # re-index the new subset

# Plot time series for fun visualization
plot(rates_m$Value ~ as.Date(rates_m$Date), type="l", main = "U.S. Treasury 10 Year - Yield (%)", xlab = "Date" ,ylab = "Yield (%)")
...
rs_m = matrix(rnorm(n_m*sims,mu,s), nrow=n, ncol=sims)
...
lambdaQTSd_m = quantile(max_stats_anova2, probs = c(0.90, 0.95, 0.975, 0.99, 0.999), na.rm = FALSE, names = FALSE, type = 1)
```
...
a_Results_m = as.data.frame(anova_F(rates$Value))
anova_results_m = data.frame(
  x = a_Results_m,
  result = c("Result", "FVal", "PVal")[c(1,2,3)]
)
# Unstack into a dataframe to see each respective index, F-Value and P-Value
anova_results_m = unstack(anova_results)

max(anova_results_m$FVal)

par(mfrow=c(2,2))
plot(rates_m$Value ~ as.Date(rates$Date), type="l", main = "U.S. Treasury 10 Year - Yield (%)", xlab = "Date" , ylab = "Yield (%)")
plot(anova_results_m$FVal, type="l", main = "F-Values Across Cascading Test", xlab = "Index", ylab = "F-Values")
plot(anova_results_m$PVal, type = "l", main = "P-Values Across Cascading Test", xlab = "Index", ylab = "P-Values")
