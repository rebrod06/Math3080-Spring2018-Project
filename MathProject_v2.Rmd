...
colnames(treasury10Y)[c(1,2)] = c("Date", "Value")
...
changepoint_search_anova(rates$Value,0)

a_result = anova_F(rates$Value)
result_index = which.max(a_result)
...
a_result2 = anova_F(sub2$Value)
result_index = which.max(a_result2)
print(rates[result_index,])


c(rates$Value[1:(2)])
c(rates$Value[(2+1):length(rates$Value)])
rep(c("a","b"), c(length(rates$Value[1:(2)]),length(rates$Value[(3):length(rates$Value)])))
