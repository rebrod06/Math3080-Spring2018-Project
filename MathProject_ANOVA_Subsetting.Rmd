...
## Step 1: Data prep
Stream U.S. 10 year Treasury data into data frame. We use daily end-of-day observations for yields. We then subset that data to take the most recent ~20 years of daily observations.
...
Import monthly rates for past 20 years

```{r, tidy = TRUE}

##using 10 year monthly rates
...
## Now run the function over every vector (column) in the matrix ##

# Load the library "snow"" to parallelize the computation using all available cores of the local machine
install.packages('CRAN', dependencies=TRUE, repos='http://cran.rstudio.com/')
library(snow)
...
max7 = anova_tests(pop3b)

# how to retrieve index from original data
# for example, grab index within original data for max5
# which(rates$Value == pop3b[max7[2]])
